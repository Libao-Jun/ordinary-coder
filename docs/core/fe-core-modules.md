...
| 规范           | 代表            | 关键特征                                        | 现状                        |
| -------------- | --------------- | ----------------------------------------------- | --------------------------- |
| IIFE           | 早期脚本        | 闭包隔离作用域，避免全局污染                    | 历史遗留，理解即可          |
| CommonJS       | Node.js         | `require` / `module.exports`，同步加载          | Node 生态主流，浏览器需打包 |
| AMD            | RequireJS       | 依赖前置、异步加载，针对浏览器                  | 基本淘汰                    |
| CMD            | Sea.js          | 就近依赖、按需执行                              | 基本淘汰（玉伯出品）        |
| UMD            | 通用包装        | 兼容 CommonJS + AMD + 全局变量                  | 库兼容旧环境仍用            |
| **ES Modules** | ECMAScript 标准 | `import` / `export`，静态可分析，浏览器原生支持 | **现代事实标准**            |

### 2. ESM 核心机制

- 静态性
> `import / export` 在编译期确定，这是 Tree Shaking（摇树，剔除未用代码）能成立的前提。
- 动态 import()
> 返回 `Promise`，实现按需加载（路由懒加载的基础）。
- 循环依赖
> ESM 有 live binding（实时绑定） 机制，与 CommonJS 的拷贝语义不同 — 修改导出方的值，导入方读到的是最新值。
- 浏览器原生
> `<script type="module">` 直接运行 ESM；
> Node.js 通过 `.mjs / package.json` 的 `type: "module"` 与 `exports` 字段支持 ESM/CJS 互操作。

### 3. 依赖治理

- **版本范围**：`^`（允许 minor / patch 升级）、`~`（仅允许 patch）。
- **锁文件**：锁定精确版本，防止"别人机器上装出来不一样"。
- **安全**：`npm audit / pnpm audit` 扫描依赖漏洞。

### 4. 代码级复用

- **组件化**
> Vue SFC（单文件组件）、React 组件（函数组件 + props）。
- **逻辑复用**
  - React Hooks（自定义 Hook 抽离状态逻辑）；
  - Vue Composables（组合式函数，等价物）。
- **资产复用**
> 工具函数库、设计系统（Design System，组件 + 令牌 + 文档的统一）。
